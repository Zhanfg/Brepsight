...
std::size_t countHierarchyNodes(
    const occ::handle<XCAFDoc_ShapeTool>& shapeTool,
    const TDF_Label& label,
    std::size_t depth = 0) {
  if (shapeTool.IsNull() || label.IsNull() || depth >= kMaxHierarchyDepth) return 0;
  std::size_t count = 1;
  ...
  if (XCAFDoc_ShapeTool::GetComponents(target, children, false)) {
    for (int index = 1; index <= children.Length(); ++index) {
      count += countHierarchyNodes(shapeTool, children.Value(index), depth + 1);
    }
  }
  return count;
}
...
IgesOcctImportResult importIgesWithOcct(const std::string& path) {
  ...
  try {
    ...
    if (!reader.Perform(path.c_str(), document)) {
      ...
      result.error = "IGESCAFControl_Reader failed to transfer the IGES document.";
      return result;
    }
    ...
    if (shapeTool.IsNull()) {
      application->Close(document);
      result.error = "IGES document has no XCAF shape tool.";
      return result;
    }
    ...
    if (freeLabels.IsEmpty()) {
      application->Close(document);
      result.error = "IGES document contains no free/root shapes.";
      return result;
    }

    auto payload = std::make_shared<OcctIgesPayload>();
    payload->document = document;
    payload->rootShapes.reserve(static_cast<std::size_t>(freeLabels.Length()));

    auto mesh = std::make_shared<MeshData>();
    mesh->sourceFormat = "iges";
    ...
    for (int index = 1; index <= freeLabels.Length(); ++index) {
      ...
      result.hierarchyNodeCount += countHierarchyNodes(shapeTool, label);
    }

    if (payload->rootShapes.empty()) {
      payload.reset();
      result.error = "IGES XCAF roots did not contain usable shapes.";
      return result;
    }
    if (mesh->vertices.empty()) {
      payload.reset();
      result.error = "IGES loaded as exact B-Rep, but display tessellation produced no triangles.";
      return result;
    }
    ...
    result.error = "Unknown OCCT exception while reading IGES.";
    return result;
  }
}
...
namespace brepsight {
IgesOcctImportResult importIgesWithOcct(const std::string&) {
  IgesOcctImportResult result;
  result.error = "BrepSight was built without the OCCT IGES provider.";
  return result;
}
}  // namespace brepsight
